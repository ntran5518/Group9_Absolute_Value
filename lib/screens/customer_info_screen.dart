/*
GROUP: 01
FILE: Customer Information Screen (Form for user to input data)
...
class CustomerInfoScreenState extends State<CustomerInfoScreen> {
  ...
  final emailRegex = RegExp('[a-zA-Z0-9.]+@(gmail|email|yahoo|outlook).[A-Za-z]{2,3}');
  ...
  void submitInformation() {
    ...
    setState(() {
      ...
      if (name.isEmpty) { // Empty name field
        nameErrors = "Customer name cannot be blank.";
        totalErrors += "$nameErrors\n";
      }
      if (email.isEmpty) { // Empty email field
        emailErrors = "Customer email cannot be blank.";
        totalErrors += emailErrors;
      } else if (!emailRegex.hasMatch(email)) { // Invalid email entered
        emailErrors = "Customer email must be in valid email format (someone@email.com).";
        totalErrors += emailErrors;
      }
      
      // check if there are errors of not
      if (totalErrors == "") { // No errors -- Customer info validated
      ...
      } else { // if errors exist, user will need empty textfields to reinput info
    ...
    return Scaffold(
      appBar: AppBar(
        // app title -- customer information input form
          title: const Text("Customer Information Form", style: TextStyle(color: Colors.white),),
          centerTitle: true,
          backgroundColor: Theme.of(context).colorScheme.primary, // follow the app color scheme
      ),
    body: Padding( 
        padding: const EdgeInsets.all(16), // add padding for layout beautification
        child: Column( // column to arrange elements vertically
          crossAxisAlignment: CrossAxisAlignment.center,
          children: [
            // Instructions for the page
            const Padding(
              padding: EdgeInsets.all(10), // add padding for layout beautification
              child: Text("Please enter your contact details to complete your purchase!", style: TextStyle(fontSize: 18, color: Colors.deepPurple),),
            ),
            Padding(
              padding: const EdgeInsets.all(10), // add padding for layout beautification
              child: TextField(
                controller: nameController, // set controller
                decoration: const InputDecoration(
                  border: OutlineInputBorder(), // border around textfield
            ...
            Padding(
                padding: const EdgeInsets.all(10), // add padding for layout beautification
                child: SwitchListTile( // user ticks whether or not they are a store member
                title: const Text("Store member?"),
                value: isMember, 
                onChanged: (bool updatedSwitchValue){
                  setState(() {
                    isMember = updatedSwitchValue; // set boolean accordingly
                  });
                }
              ),
            ),
            Padding(
                padding: const EdgeInsets.all(10), // add padding for layout beautification
                child: CheckboxListTile( // user checks if they want their receipt emailed to them
                ...
                onChanged: (bool? updatedCheckValue){
                  setState(() {
                    if (updatedCheckValue != null) {
                      getEmailedReceipt = updatedCheckValue; // set boolean accordingly
                    }
                  });
                },
              ),
            ),
            FilledButton( // button to submit user info & finish purchase
            ...
            Text(totalErrors, style: const TextStyle(color: Colors.red), textAlign: TextAlign.center,),
        ])
    )
    );
  }
